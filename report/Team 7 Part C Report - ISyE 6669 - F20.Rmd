...
z_{rk} = \sum^{N}_{j=1} \sum^{H}_{i=1} x_{ijk} \forall r \forall k \text{ if warehouse j in region r and order i in r  }\\
z^{'}_{rk}  = \sum^{N}_{j=1} \sum^{H}_{i=1} x_{ijk} \forall r \forall k \text{ if warehouse j not in region r and order i in r   }\\
M y_{ij} \geq \sum_k x_{ijk} \forall j \in N, \forall i \in H. 
\end{flalign}  

\begin{flalign}
M > 0, \text{ arbitrary large positive number (i.e. 'Big M').}  \\
x_{ijk}, d_{ik}, s_{jk} \in \mathbb{N}\
